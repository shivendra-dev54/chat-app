...
export const Chat = () => {
  ...
  // Fetch previous chats & user details
  useEffect(() => {
    const fetchChatData = async () => {
      ...
      try {
        // ✅ Fetch other user
        const resUser = await axios_helper({
          url: `http://localhost:64000/api/user/${id}`,
          method: "GET",
          credentials: true,
          body: {},
        });
        ...
        // ✅ Fetch chat history
        const resChats = await axios_helper({
          url: "http://localhost:64000/api/chat/get",
          method: "POST",
          credentials: true,
          body: {
            sender_id: user.id,
  ...
  const handleDelete = async (msg: ChatMessage) => {
    try {
      const res = await axios_helper({
        url: `http://localhost:64000/api/chat/delete/${msg.id}`,
        method: "DELETE",
        credentials: true,
        body: {},
      });
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-blue-950 via-black to-indigo-950 flex flex-col">
      ...
      <div className="flex-1 p-6 space-y-4 overflow-y-auto">
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`relative max-w-lg p-3 rounded-2xl shadow-md group ${msg.from === "self"
...

            {
              (msg.sender_id === user?.id) && (
                <button
                  ...
                  className="absolute top-2 right-2 p-1 rounded-full hover:bg-red-500/20"
                >
